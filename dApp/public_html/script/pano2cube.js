...
class CubeFace {
  ...
  setDownload(url, fileExtension) {
    ...
    fd.append('url', url);
    try{
        var url_data = document.framework.rootDir + '/cubemap.php?name=' + name + '&img_id=' + parseInt($id("cubemap_id").value);
    }catch(e){
        var url_data = document.framework.rootDir + '/cubemap.php?name=' + name;
    }
    $.ajax({
        type: 'POST',
        url: url_data,
...
function renderFace(data, faceName, position) {
  ...
  const worker = new Worker('/script/convert.js');
