<?php
/**
* Prints Coinpayments form.
* @path /engine/core/function/print_coinpayments_form.php
* 
...
* @param double $sum Amount to pay via Coinpayments in USD.
...
* @usage <code> 
*   $return = $_SERVER["PUBLIC_URL"]."/";
*   engine::print_coinpayments_form(1, 100, $return); 
* </code>
*/
function print_coinpayments_form($invoice_id, $sum, $return, $autopay=0){
    if(empty($_SESSION["user"]["email"])) return engine::error(401);
    if(strpos("http", $return) != 0){
        $return = $_SERVER["PUBLIC_URL"] .$return;
    }
    $query = 'SELECT `value` FROM `nodes_config` WHERE `name` = "coinpayment_private"';
    $res = engine::mysql($query);
    $data = mysqli_fetch_array($res);
    $private = $data["value"];
    $query = 'SELECT `value` FROM `nodes_config` WHERE `name` = "coinpayment_public"';
    $res = engine::mysql($query);
    $data = mysqli_fetch_array($res);
    $public = $data["value"];
    $uniq = rand(100000, 1000000);
    $secret = md5($_SESSION["user"]["id"].'x'.$invoice_id.'x'.$uniq);
    $query = 'INSERT INTO `nodes_transaction`(user_id, invoice_id, order_id, amount, `txn_id`, `payment_date`, status, date, gateway, comment, ip) '
            . 'VALUES("'.$_SESSION["user"]["id"].'", "'.$invoice_id.'", "-1", "'.$sum.'", "'.$uniq.'", "'.$datetime.'", "-1", "'.date("U").'", "Coinpayments", "Temp", "'.$_SERVER["REMOTE_ADDR"].'")';
    engine::mysql($query);
    $ipn_url = $_SERVER["PUBLIC_URL"] ."coinpayments.php?invoice_id=".$invoice_id.'&secret='.$secret;
    $cps = new CoinPaymentsAPI();
    $cps->Setup($private, $public);
    $result = $cps->CreateTransactionSimple($sum, 'USD', 'BTC', '', $ipn_url, $_SESSION["user"]["email"]);
    if(!empty($result['result']['status_url'])){
        $fout = '<a vr-control id="btc_process" href="'.$result['result']['status_url'].'" class="btn w280">'.lang("Make a payment").'</a>';
    }else{
        $fout = '<a vr-control id="btc_process" onClick=\'alert("'.$result['error'].'");\' class="btn w280">'.lang("Make a payment").'</a>';
    }
    if($autopay) die('<script>window.location = "'.$result['result']['status_url'].'";</script>');
    return $fout;
}
