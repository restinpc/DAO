<?php
/**
* Prints VR panorama level plan block.
* @path /engine/core/function/print_level_plan.php
*
...
function print_level_plan($level_id) {
    engine::log('function.print_level_plan('.$level_id.')');
    try {
        ...
        while ($data = mysqli_fetch_array($res)) {
            ...
            if ($left == null || $data["lat"] < $left) {
                $left = $data["lat"];
            }
            if ($right == null || $data["lat"] > $right) {
                $right = $data["lat"];
            }
            if ($top == null || $data["lng"] < $top) {
                $top = $data["lng"];
            }
            if ($bottom == null || $data["lng"] > $bottom) {
                $bottom = $data["lng"];
            }
        }
        ...
        foreach ($scenes as $scene) {
            ...
            $fout .= '<img id="camera_icon_'.$scene["id"].'"
                    ...
                    title="'.$scene["name"].'" 
                    onClick=\'
                        parent.document.framework.hideMap();
        ...
        engine::throw('function.print_level_plan('.$level_id.')', $e);
    }
}
