...
function print_product_preview($site, $data, $right=0) {
    engine::log('product.print_product_preview('.json_encode($data).')');
    try {
        $images = explode(";", $data["img"]);
        if (empty($images[0])) {
            $image = $_SERVER["DIR"].'/img/no-image.jpg';
        } else {
            $image = $_SERVER["DIR"].'/img/data/thumb/'.$images[0];
        }
        ...
        engine::throw('product.print_product_preview('.json_encode($data).')', $e);
    }
}
