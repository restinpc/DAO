...
function print_admin($cms) {
    ...
    try {
        ...
        while ($data = mysqli_fetch_array($res)) {
            if ($data["access"]) {
                $fout .= '<div class="admin_menu_icon">
                    <a id="admin-link-'.$data["id"].'" href="'.$_SERVER["DIR"].'/admin/?mode='.$data["url"].'">
                        <img src="'.$_SERVER["DIR"].'/img/'.$data["img"].'" />
                        <br/>
                        '.engine::lang($data["name"]).'
                    </a>
                </div>';
            }
        }
        ...
        engine::throw('admin.print_admin()', $e);
    }
}
