...
function print_admin_errors($cms) {
    engine::log('admin.print_admin_errors()');
    try {
        ...
        $query = 'SELECT * FROM `nodes_error` WHERE `display` = 1 ORDER BY `'.$_SESSION["order"].'` '.$_SESSION["method"].' LIMIT '.($from-1).', '.$_SESSION["count"];
        $requery = 'SELECT COUNT(*) FROM `nodes_error` WHERE `display` = 1';
        ...
        engine::throw('admin.print_admin_errors()', $e);
    }
}
