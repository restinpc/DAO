<?php
/**
* Prints VR panorama level plan editor.
* @path /engine/core/pano/pano_level_plan.php
*
...
function pano_level_plan($level_id) {
    engine::log('pano.pano_level_plan('.$level_id.')');
    try {
        $query = 'SELECT * FROM `nodes_vr_level` WHERE `id` = "'.$level_id.'"';
        $res = engine::mysql($query);
        $level = mysqli_fetch_array($res);
        $fout = '<div style="height:100px;" class="center">'.engine::lang("Drag-n-drop points to locate scenes on level").'<br/>
            '.engine::lang("Double click to open scene preview").'</div>
            <div id="level_plan_container" ondragover="event.preventDefault();">
        ...
        while ($data = mysqli_fetch_array($res)) {
            ...
            if ($data["lat"] < $left || $left == null) {
                $left = $data["lat"];
            }
            if ($data["lat"] > $right || $right == null) {
                $right = $data["lat"];
            }
            if ($data["lng"] < $top || $top == null) {
                $top = $data["lng"];
            }
            if ($data["lng"] > $bottom || $bottom == null) {
                $bottom = $data["lng"];
            }
        }
        $points = '';
        ...
        $fout .= '</div>
            ...
            <form method="POST" id="objects" style="width: 320px; margin:0px auto; text-align:left;" ENCTYPE="multipart/form-data">
                ...
                <br/>
                <input type="button" onClick=\'document.admin.levelApplyChages();\' class="btn w280" value="'.engine::lang("Apply changes").'" />
                <br/><br/>
        ...
        engine::throw('pano.pano_level_plan('.$level_id.')', $e);
    }
}
