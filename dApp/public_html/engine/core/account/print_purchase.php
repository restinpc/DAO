    ...
    while($d = mysqli_fetch_array($r)){  
        if($d["count"]>0){
            ...
            if($d["status"]==0){
                ...
                $status = lang('Sended');
                $button = '<a vr-control id="link-confirm-'.$d["id"].'" href="'.$_SERVER["DIR"].'/account/confirm/'.$d["id"].'"><input type="button" class="btn confirm_receipt" value="'.lang("Confirm receipt").'" /></a>';
            }else{
            ...
            $fout .= '<div class="print_order">
                ...
                <div>
                    ...
                    '.lang("Seller").': <a vr-control id="link-user-'.$user["id"].'" href="'.$_SERVER["DIR"].'/account/inbox/'.$user["id"].'" target="_blank">'.$user["name"].'</a><br/><br/>'
                    .$track.
