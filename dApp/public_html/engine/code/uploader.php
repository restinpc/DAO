    ...
    if (!empty($_POST["url"])) {
        ...
        if (rename($file, $_SERVER["DOCUMENT_ROOT"].$_SERVER["DIR"]."/".$f_name)) {
            ...
            $fout = '<body class="nodes uploader_body" draggable="false" title="'.engine::lang("For uploading selected area use double click").'">
                ...
                <div id="frame" draggable="false" onDragStart="return false;" style="width:'.($THUWIDTH/ $scale).'px; height:'.($THUHEIGHT/ $scale).'px;position: absolute;top: 0px;left: 0px;display:block;">
