...
function uploader() {
    engine::log('uploader('.json_encode($_GET).', '.json_encode($_POST).')');
    try {
        ...
        try {
            ...
            if (img.value != "") {
                ...
                if ($ext != "jpeg" && $ext != "jpg" && $ext != "png" && $ext != "gif") {
                     die(engine::lang("Error").'<script type="text/javascript">setTimeout(() => { window.location = "'.$_SERVER["DIR"].'/uploader.php?id='.$_GET["id"].'"; }, 1000);</script>');
                }
                if ($ext == "jpeg") {
                    $ext = "jpg";
                }
                ...
                if (rename($file, $_SERVER["DOCUMENT_ROOT"].$_SERVER["DIR"]."/".$f_name)) {
                    ...
                    $scale = 1;
                    $fout = '
<body class="nodes uploader_body" draggable="false" title="'.engine::lang("For uploading selected area use double click").'">
    ...
    <script type="text/javascript">
        ...
        try{
            ...
            $fout = '
<body class="nodes dragndrop_body"> 
    ...
    <form method="POST" id="new_image_form">
        ...
        engine::throw('uploader('.json_encode($_GET).', '.json_encode($_POST).')', $e);
    }
}
