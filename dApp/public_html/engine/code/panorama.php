...
function panorama() {
    engine::log('panorama('.json_encode($_GET).', '.json_encode($_POST).')');
    try {
        ...
        if (!empty($_GET["id"])) {
            ...
            while ($d = mysqli_fetch_array($res)) {
                $objects .= engine::pano_print_object($site, $d);
            }
            $new_obj = array();
            $new_obj["id"] = "new_obj";
            ...
            $new_obj["text"] = "";
            $objects .= engine::pano_print_object($site, $new_obj, 1);
            ...
            $navigation .= engine::pano_print_navigation($site, $new_nav, 1);
            $query = 'SELECT * FROM `nodes_vr_link` WHERE `scene_id` = "'.$data["id"].'"';
            $res = engine::mysql($query);
            while ($d = mysqli_fetch_array($res)) {
                $gsv .= engine::pano_print_link($site, $d);
            }
            $new_nav = array();
            ...
            $new_nav["level_id"] = $data["level_id"];
            $new_nav["scale"] = "10 10 10";
            $gsv .= engine::pano_print_link($site, $new_nav, 1);
            $fout .= $site->content;
            $fout .= '</a-entity>
...
<body class="nodes">
    ...
    <script>
        ...
        engine::throw('panorama('.json_encode($_GET).', '.json_encode($_POST).')', $e);
    }
}
