...
function register($site) {
    ...
    try {
        ...
        $site->title = engine::lang("Sign Up");
        if ($_SESSION["Lang"] == "en") {
            $site->keywords = array(
                "Registration",
                "Web 3.0",
            ...
            $site->description = "Registering as a Member of the Web 3.0 Community";
            ...
            $site->description = '註冊成為 Web 3.0 社區的成員';
        } else {
            $site->keywords = array(
                "Регистрация",
                "Web 3.0",
            ...
            $site->description = "Регистрация в качестве участника Web 3.0 сообщества";
        }
        if (!empty($_POST["email"]) && !empty($_POST["pass"]) && !empty($_POST["telegram"])) {
            if ($_POST["captcha"] != $_SESSION["captcha"]) {
                $site->onload .= ' alert("'.engine::lang("Error").'. '.engine::lang("Invalid conformation code").'."); ';
            } else {
                ...
                if (!empty($d)) {
                    $site->onload .= ' alert("'.engine::lang("Error").'. '.engine::lang("Email").' '.engine::lang("already exist").'."); ';
                    unset($_POST["email"]);
                    ...
                    if (empty($_SESSION["redirect"])) {
                        ...
                        $site->onload .= 'window.location = "'.$_SERVER["DIR"].'/account";';
                    } else {
                        $site->content = '<script language="JavaScript">setTimeout(() => { window.location = "'.$_SESSION["redirect"].'"; }, 1);</script>';
                    }
                    return;
                } else {
                    $site->onload .= ' alert("'.engine::lang("Error").'. '.engine::lang("Incorrect email").strpos($email, "@").'."); ';
                    unset($_POST["email"]);
                }
            }
        } else {
            $site->onload .= '$("input-email").focus();';
            $site->content = '<script>
